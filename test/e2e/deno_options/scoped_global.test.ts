...
Deno.test("run script with scoped options", async () => {
  await testDenoXRun('start', 'test/fixture/script_permission', async ({ code, output }) => {
    assertEquals(code, 0);
...
Deno.test("run script with global options", async () => {
  await testDenoXRun('start', 'test/fixture/global_permission', async ({ code, output }) => {
    assertEquals(code, 0);
    assertStrContains(output, "I'm text file content");
  });
});
...
Deno.test("run script with scoped and global options", async () => {
  await testDenoXRun('start', 'test/fixture/script_global_permission', async ({ code, output }) => {
    assertEquals(code, 0);
    assertStrContains(output, "I'm text file content");
...
Deno.test("run script with erroneous merged scoped and global options", async () => {
  await testDenoXRun('start', 'test/fixture/merged_scoped_global__invalid_permission', async ({ code, errOutput }) => {
    assertEquals(code, 1);
    assertStrContains(
      errOutput,
