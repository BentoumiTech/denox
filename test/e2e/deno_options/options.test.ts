...
Deno.test("test permissions are applied", async () => {
  await testDenoXRun("permissions", "test/fixture/deno_options", async ({ output, code }) => {
    assertEquals(code, 0);
...
Deno.test("test all permissions are applied", async () => {
  await testDenoXRun("all-permissions", "test/fixture/deno_options", async ({ output, code }) => {
    assertEquals(code, 0);
...
Deno.test("test seed option is applied", async () => {
  await testDenoXRun("seed", "test/fixture/deno_options", async ({ output, code }) => {
    assertEquals(code, 0);
...
Deno.test("test quiet option is applied", async () => {
  await testDenoXRun("quiet", "test/fixture/deno_options", async ({ output, code }) => {
    assertEquals(code, 0);
...
Deno.test("test log-level option is applied", async () => {
  await testDenoXRun("log-level", "test/fixture/deno_options", async ({ code, output }) => {
    assertEquals(code, 0);
...
Deno.test("test import map option is applied", async () => {
  await testDenoXRun("import-map", "test/fixture/deno_options", async ({ code, errOutput }) => {
    assertEquals(code, 0);
...
Deno.test("test v8-flags, cached-only, cert, no-remote, reload options do not crash", async () => {
  await testDenoXRun("rest-options", "test/fixture/deno_options", async ({ code }) => {
    assertEquals(code, 0);
  });
});
